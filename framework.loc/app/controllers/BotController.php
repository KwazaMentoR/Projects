...
// если стоит пустой контроллер, то не пересылает на index.php
    ...
    {
        $path = "https://yobit.net/api/3/info";
        $info = file_get_contents($path);
        $obj_info = [];
        if ($info) {
            $obj_info = json_decode($info, true); // второй параметр true возвращает ассоц. массив
        }
        return $obj_info;
    }

    public function botAction() //метод передает данные с ёбита по ajax запросу
    {
        $pairs = !empty($_GET['pairs']) ? $_GET['pairs'] : null;
        $path = "https://yobit.net/api/3/trades/{$pairs}?limit=10";
        $data = file_get_contents($path); //приходят данные в виде строки
    ...
    public function getApiAction($method = null, $req = [])
    {
    ...
    public function tradeAction()
    {
        ...
        $etherscan = $this->cryptocompare($serv_time);
        //debug($etherscan);
        ...
        if ($ordersell){
            ...
            if ($buyinfo['status'] === 1 && $sellinfo['status'] === 1 && $sellinfo['timestamp_created'] > $buyinfo['timestamp_created']) {
                // время проданного учесть
                // здесь записываем в отдельную БД значения последних успешных сделок покупки/продажи,
                // что бы потом посчитать разность и использовать в торгах торгуемую сумму + навар
                // Отмененные частично выполненные ордера записываются в эту БД по мере их отмены
                // В любом случае покупка начнется только после status=1 у продажи и отмены,
                // смежные отмененные и выполненные ордеры запишутся в таблицу без преувеличения с одной или другой стороны

                $buyprof = $buyinfo['start_amount'] * $buyinfo['rate']*1.002;
                $sellprof = $sellinfo['start_amount'] * $sellinfo['rate'] * 0.998;
                $orderbuyprof = Db::query("SELECT id_order FROM profit WHERE type = ? ORDER BY id DESC LIMIT 1",['buy']); // берем id покупки из profit
        ...
        $part = !empty($_GET['lot']) ? $_GET['lot'] : null; //$part равен значению lot, если оно не пустое иначе null. На сколько рублей закупать
        $cash = !empty($_GET['percent']) ? $_GET['percent'] : null; //Накрутка к продаже, например 1%
        $profcheck = !empty($_GET['profit']) ? $_GET['profit'] : null;
...
*/
        ...
        if (!empty($orderbuy)){ //если есть запись в БД об ордере на покупку
            if (!empty($buyinfo)){
                if ($buyinfo['status'] != 0) { // если нет активных ордеров на закупку
                    ...
                    //может быть так, что есть getKey('buy'), значит купили, но $coinfunds == 0 ибо yobit тормозит и тогда проверяем была ли продажа
                    // и если да, то сравниеваем время продажи и покупки
                        ...
                        if (!empty($ordersell)){
                            ...
                            if ($sellinfo['status'] === 1) { //нет активных ордеров на продажу, ошибка yobit уже не пройдет
                                if ($sellinfo['timestamp_created'] > $buyinfo['timestamp_created']) { //и время продажи позже чем покупки
                                    ...
                                    $gettrade = $this->buyCoin($part, $first, $middle, $stat, $differ, $savetime, $serv_time, $params, '', $ordersell['id_order'], $etherscan);
                            ...
                            if ($sellinfo['status'] === 0 || $sellinfo['status'] === 2 || $sellinfo['status'] === 3) { //если лот продажи активен или отменен
                                ...
                                if (($etherscan - 250) < $middle && $etherscan > $middle) {
                                    // При падении РОСТА на этерскан до определенного значения
                                    if ($sellinfo['rate'] > $buyinfo['rate'] + $buyinfo['rate'] * $cash / 2) {
                                        //проверка на случай если мы уже поставили ордер с половиной навара
                                        ...
                                        if ($cancelorder->success === 1 || $sellinfo['status'] === 2) {
                                            // здесь status === 2 на случай если $gettrade->success === 1 не сработает
                                            if ($buyinfo['rate'] < $first) {
                                                //если цена купленного лота все еще меньше первого из стакана, то продаем  с половиной навара
                                                $cash = $cash / 2;
                                            } else {
                                                //по какой купили по такой и продаем без навара, что бы не зависнуть при падении на бирже
                                                $cash = 0;
                                            }
                                            ...
                                            if ($sellinfo['status'] === 3) {
                                                //находится не в $gettrade->success === 1, потому что если новая продажа не сработает, то не запишется profit
                                                // плюсуем в profit отмененный частично выполненный ордер
                                                $sellprof = ($sellinfo['start_amount'] - $sellinfo['amount']) * $sellinfo['rate'] * 0.998;
                                                Db::myExecute("UPDATE profit SET profit = profit + ?, id_order = ? WHERE type = ?", [$sellprof, $ordersell['id_order'], $sellinfo['type'],]);
                                            }
                                        }else{
                    ...
                    // Значит скорей всего когда выполнится, то приобретет статус 1 - исполнен
                ...
                if ($buyinfo['status'] === 0 || $buyinfo['status'] === 2 || $buyinfo['status'] === 3) {
                    ...
                    if ($first < $botspam && $first > $buyinfo['rate']){ // например 12005 < 12015 но 12005 > 12000
                        $this->checkBuy($orderbuy, $buyinfo, $part, $first, $middle, $stat, $savetime, $serv_time, $params, $etherscan);
                    }else{
                        if ($serv_time > $buyinfo['timestamp_created'] + $orderwait || $middle > $highprice || $etherscan < $middle - 350){
                            //если прошло 30 минут и ордер все еще существует или средняя цена из стакана выросла больше
                            // чем на 50 рублей от $buyinfo['rate'] или на этерскане ПАДЕНИЕ больше 350 руб
                            $this->checkBuy($orderbuy, $buyinfo, $part, $first, $middle, $stat, $savetime, $serv_time, $params, $etherscan);
                        }else{
            ...
            if ($coinfunds === 0) {
                $gettrade = $this->buyCoin($part, $first, $middle, $stat, $differ=0, $savetime, $serv_time, $params, $amount = '', 0, $etherscan);
            }
        }

        debug($etherscan);
        $color = $etherscan < $middle - 350 ? 'color:red' : '';
        echo "<div style='$color'> Разница с Etherscan:" . $etherscan . "</div>";
    ...
    public function buyCoin($part,$first,$middle,$stat,$differ,$savetime,$serv_time,$params,$amount = '',$id_order = 0, $etherscan)
        // метод покупки с условиями
    {
        $gettrade = (object)['success' => (int)0];
        if ($etherscan > $middle - 350){ // сравнение с курсом на этерскане. если курс на этекрсане УПАЛ то не покупаем
            if ($middle < $stat*1.01 || $middle > $savetime['value']+350 || ($etherscan - 250) > $middle /*$middle < $differ*/) {
                //не сработает только если $middle больше $stat и нет резкого скачка
                // ($this->etherscan() - 250) > $middle При РОСТЕ курса на этерскане и разнице с йобит
                if ($first < ($middle + 70)) { //здесь берем по первому из стакана,но если он не сильно отрывается от среднего знач
    ...
    public function checkBuy($orderbuy, $buyinfo, $part, $first, $middle, $stat, $savetime, $serv_time, $params, $etherscan)
    {
        $cancelorder = (object)['success' => (int)0];
        if ($buyinfo['status'] != 2){
        ...
        if ($cancelorder->success === 1 || $buyinfo['status'] === 2) {
            ...
            $gettrade = $this->buyCoin($part, $first, $middle, $stat, $differ = 0, $savetime, $serv_time, $params, $buyinfo['amount'], 0, $etherscan);
            ...
            if ($gettrade->success === 1) {
                ...
                if ($buyinfo['status'] === 2 || $buyinfo['status'] === 3) {
                    ...
                    $buyprof = ($buyinfo['start_amount'] - $buyinfo['amount']) * $buyinfo['rate'] * 1.002;
                    Db::myExecute("UPDATE profit SET profit = profit + ?, id_order = ? WHERE type = ?", [$buyprof, $orderbuy['id_order'], $buyinfo['type'],]);
                }
            }else{
    ...
    public function getTrade($method, $params, $serv_time, $type, $price) // непосредственная операция покупки/продажи
    {
        ...
        // здесь может произойти баг и $gettrade вернуть ошибку, тогда перед записью в БД надо это проверить
    ...
    public function getCircle ($serv_time, $middle)
    {
    ...
    public function saveTime($serv_time, $middle)
    {
        $cource = Db::query("SELECT time, value FROM cource WHERE type = ?",['yobit']);
    ...
    {
        $timeover = 86400; // время через которое удаляем записи
    ...
    public function etherscan() //извлечение информации с сайта etherscan о курсе eth
    {
    ...
    public function cryptocompare($serv_time) //извлечение информации с сайта etherscan о курсе eth
    {
        ...
        $time = $compare[0]['time'] + 25; // записываем значение eth с compare
        if ($serv_time > $time){
    ...
    function getCourseUsd() //получение курса доллара
    {
